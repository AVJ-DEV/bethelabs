...
if (empty($error) && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create') {
    try {
        ...
        if ($id) {
            AuthController::log('create', 'team', 'Membre d\'équipe créé ID: ' . $id);
            $success = 'Membre d\'équipe créé avec succès.';
        } else {
            $error = implode('<br>', $teamModel->getErrors());
        }
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
...
if (empty($error) && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update') {
    try {
        ...
        if ($teamModel->update($id, $data)) {
            AuthController::log('update', 'team', 'Membre d\'équipe modifié ID: ' . $id);
            $success = 'Membre d\'équipe modifié avec succès.';
        } else {
            $error = implode('<br>', $teamModel->getErrors());
        }
    } catch (Exception $e) {
...
if (empty($error) && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    try {
        $id = $_POST['id'];
        $teamModel->delete($id);
        AuthController::log('delete', 'team', 'Membre d\'équipe supprimé ID: ' . $id);
...
$allTeam = $teamModel->getAll('created_at DESC');
$editingTeam = null;

// Check if editing
if (isset($_GET['edit'])) {
    $editingTeam = $teamModel->getById($_GET['edit']);
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Équipe - BetheLabs Admin</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <nav class="top-navbar d-flex justify-content-between align-items-center">
            <div>
                <h5 class="mb-0"><i class="bi bi-people me-2"></i>Gestion de l'Équipe</h5>
            </div>
            <div>
        ...
        <div class="content-wrapper">
            ...
            <div class="form-card">
                <h5 class="mb-4">
                    <?php echo $editingTeam ? 'Modifier le membre' : 'Ajouter un membre'; ?>
                </h5>
                
                <form method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                    <?php echo \Csrf::inputField(); ?>
                    <input type="hidden" name="action" value="<?php echo $editingTeam ? 'update' : 'create'; ?>">
                    <?php if ($editingTeam): ?>
                        <input type="hidden" name="id" value="<?php echo $editingTeam['id']; ?>">
                    <?php endif; ?>
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="mb-3">
                                ...
                                <input type="text" name="name" class="form-control" required 
                                       value="<?php echo $editingTeam ? htmlspecialchars($editingTeam['name']) : ''; ?>">
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Poste *</label>
                                <input type="text" name="position" class="form-control" required 
                                       value="<?php echo $editingTeam ? htmlspecialchars($editingTeam['position']) : ''; ?>">
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Spécialité</label>
                                <input type="text" name="speciality" class="form-control" 
                                       value="<?php echo $editingTeam ? htmlspecialchars($editingTeam['speciality']) : ''; ?>">
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Biographie</label>
                                <textarea name="bio" class="form-control" rows="5"><?php echo $editingTeam ? htmlspecialchars($editingTeam['bio']) : ''; ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <?php if ($editingTeam && !empty($editingTeam['image'])): ?>
                                    <small class="text-muted d-block mt-2">
                                        <i class="bi bi-check-circle text-success"></i> Image actuelle: <a href="<?php echo htmlspecialchars($editingTeam['image']); ?>" target="_blank">voir</a>
                                    </small>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            ...
                            <?php echo $editingTeam ? 'Mettre à jour' : 'Ajouter'; ?>
                        </button>
                        <?php if ($editingTeam): ?>
            ...
            <div class="table-card">
                <div class="card-header bg-light p-4">
                    <h5 class="mb-0"><i class="bi bi-list me-2"></i>Membres de l'équipe (<?php echo count($allTeam); ?>)</h5>
                </div>
                            ...
                            <?php if (!empty($allTeam)): ?>
                                <?php foreach ($allTeam as $member): ?>
                                    <tr>
                                        ...
                                        <td><small><?php echo date('d/m/Y', strtotime($member['created_at'])); ?></small></td>
                                        <td>
                                            <a href="?edit=<?php echo $member['id']; ?>" class="btn btn-sm btn-primary" title="Modifier">
